...
OPTION( ere,               0,  0,  1,0,1,0, "ERE") \
OPTION( erebackward,       0,  0,  1,0,0,1, "look for redundancies by backward subsumption instead") \
OPTION( ereclslim,       1e2,  0,2e9,0,0,1, "ERE resolvent size limit") \
OPTION( ereeffort,         5,  0,1e6,0,0,1, "relative effort per mille") \
OPTION( ereirredonly,      0,  0,  1,0,0,1, "only resolve irredundant clauses") \
OPTION( eremarkedonly,     0,  0,  1,0,0,1, "only resolve on variables marked for subsumption") \
OPTION( ereocclim,       1e3,  0,2e9,0,0,1, "ERE occurrence list limit") \
OPTION( ereselfsub,        0,  0,  0,0,0,1, "check for self subsumption") \
OPTION( erethresh,        10,  0,100,1,0,1, "delay if ticks smaller thresh*clauses") \
